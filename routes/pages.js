...
router.get('/demo', async function (req, res) {
    ...
    let timestamp = parseInt(Date.now()/1000);
    //注意参数名全是小写
    let str = [`noncestr=${nonceStr}`,`jsapi_ticket=${ticket}`,`timestamp=${timestamp}`,`url=http://b1aac1e6.ngrok.io/pages/demo`].sort().join('&');
    let signature  =  crypto.createHash('sha1').update(str).digest('hex');
    res.render('demo',{
        appId:appID,
        timestamp,
...
router.get('/newpage', async function(req, res) {
    ...
    let nonceStr = Math.random().toString(36).substr(2,15);
    //注意参数名全是小写
    let str = [`noncestr=${nonceStr}`,`jsapi_ticket=${ticket}`,`timestamp=${timestamp}`,`url=http://8b03d49e.ngrok.io/pages/newpage`].sort().join('&');
    let signature  =  crypto.createHash('sha1').update(str).digest('hex');
    res.render('newpage', {
        appId: appID,
        timestamp,
